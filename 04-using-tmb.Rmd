...
Figure \@ref(fig:3tinnitus-fig) presents the raw data in the [Table](#table:tinnitus_data) below, which are also available for download at *[data/tinnitus.RData](#tinnitus.RData)*.
...
Then,
\[
\text{P}(X_{n+h} = x \vert X^{(n)} = x^{(n)}) = \frac{\b{\alpha}_n \bs{\Gamma}^h \textbf{P}(x) \bs{1}'}{\b{\alpha}_n \bs{1}'} = \bs{\Phi}_n \bs{\Gamma}^h \textbf{P}(x) \bs{1}'
\]
