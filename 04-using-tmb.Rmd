...
The function is defined in *[code/poi_hmm.cpp](code/poi_hmm.cpp)*
```{Rcpp 4poi_hmm.cpp, code=readLines("code/poi_hmm.cpp"), eval=FALSE}
```

## Preparing data and functions <br/><span style="font-size:200%;color:red">SONDRE</span>
...
Figure \@ref(fig:4tinnitus-fig) present the raw [data](#table:tinnitus_data), which are also available for download at *[data/tinnitus.RData](data/tinnitus.Rdata)*.

<span style="font-size:400%;color:red">SONDRE</span>
I don't think we can keep the 2 tables. I prefer the first one. What do you think?\
Also, do you think it is better to link files to their entry in [Directory structure] or to keep it like this and always link to the download links?
...
temp <- xtable(temp,
               ...
               label = "table:tinnitus_data") # reference via [some_text](#table:tinnitus_data)
print(temp, include.rownames = FALSE, include.colnames = FALSE,
      hline.after = c(0, nrow(temp)),
      type = "html")
```

Table \@ref(tab:4tinnitus-table2) and Figure \@ref(fig:4tinnitus-fig) presents the raw data, which are also available for download in the data folder on *[data/tinnitus.RData](data/tinnitus.Rdata)*.

```{r 4tinnitus-table2, echo = FALSE, warning = FALSE}
temp <- matrix(c(tinn_data,rep("", 3)), nrow = 6, byrow = TRUE)
knitr::kable(temp, booktabs = TRUE,
  caption = "TYT data. Observations collected by the TYT app on 87 successive days (from left to right).")
```
...
It is noteworthy that more complex constraints are possible as well
For example, to impose equality constraints (such as $\gamma_{11} = \gamma_{22}$), the corresponding factor level has to be identical for the concerned entries.
...
However, it appears that it can only be done in general when the constraint involves natural parameters of the same row with the exception of a 2 state model.
