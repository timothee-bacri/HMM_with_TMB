unlink("full_log_simu1.txt")
sink("full_log_simu1.txt")
...
m <- M_LIST_SIMU1
if (m == 2) {
  simu1_true_gamma <- matrix(c(0.95, 0.05,
                               0.15, 0.85),
                             byrow = TRUE,
                             nrow = m,
                             ncol = m)
} else if (m == 3) {
  simu1_true_gamma <- matrix(c(0.950, 0.025, 0.025,
                               0.050, 0.900, 0.050,
                               0.075, 0.075, 0.850),
                             byrow = TRUE,
                             nrow = m,
  ...
  simu1_true_gamma <- matrix(c(0.850, 0.050, 0.050, 0.05,
...
simu1_true_lambda <- seq(1,
                         7,
                         length.out = m)
simu1_true_delta <- stat.dist(simu1_true_gamma)

simu1_data <- pois.HMM.generate.estimable.sample(ns = DATA_SIZE_SIMU1,
                                                 mod = list(m = m,
                                                            lambda = simu1_true_lambda,
                                                            gamma = simu1_true_gamma,
                                                            delta = simu1_true_delta),
                                                 debug_message = "simu1_data")
simu1_states <- simu1_data$states
simu1_data <- simu1_data$data
...
working_params_init <- pois.HMM.pn2pw(m,
                                      lambda_init,
                                      gamma_init)
# working_params_init$simu1_true_lambda <- simu1_true_lambda
# working_params_init$simu1_true_gamma <- simu1_true_gamma
# working_params_init$simu1_true_delta <- simu1_true_delta

TMB_data <- list(x = simu1_data,
                 m = m)
...
if (result_BFGS$convergence == FALSE) {
  stop(paste("BFGS didn't converge properly, simu1 dataset, m =", m))
}
if (result_BFGS_gr$convergence == FALSE) {
  stop(paste("BFGS with gradient didn't converge properly, simu1 dataset, m =", m))
}
if (result_L_BFGS_B$convergence == FALSE) {
  stop(paste("L_BFGS_B didn't converge properly, simu1 dataset, m =", m))
}
if (result_L_BFGS_B_gr$convergence == FALSE) {
  stop(paste("L_BFGS_B with gradient didn't converge properly, simu1 dataset, m =", m))
}
if (result_CG$convergence == FALSE) {
  stop(paste("CG didn't converge properly, simu1 dataset, m =", m))
}
if (result_CG_gr$convergence == FALSE) {
  stop(paste("CG with gradient didn't converge properly, simu1 dataset, m =", m))
}
if (result_Nelder_Mead$convergence == FALSE) {
  stop(paste("Nelder_Mead didn't converge properly, simu1 dataset, m =", m))
}
if (result_nlm$convergence == FALSE) {
  stop(paste("nlm didn't converge properly, simu1 dataset, m =", m))
}
if (result_nlm_gr$convergence == FALSE) {
  stop(paste("nlm with gradient didn't converge properly, simu1 dataset, m =", m))
}
if (result_nlm_he$convergence == FALSE) {
  stop(paste("nlm with hessian didn't converge properly, simu1 dataset, m =", m))
}
if (result_nlm_grhe$convergence == FALSE) {
  stop(paste("nlm with gradient + hessian didn't converge properly, simu1 dataset, m =", m))
}
if (result_nlminb$convergence == FALSE) {
  stop(paste("nlminb didn't converge properly, simu1 dataset, m =", m))
}
if (result_nlminb_gr$convergence == FALSE) {
  stop(paste("nlminb with gradient didn't converge properly, simu1 dataset, m =", m))
}
if (result_nlminb_he$convergence == FALSE) {
  stop(paste("nlminb with hessian didn't converge properly, simu1 dataset, m =", m))
}
if (result_nlminb_grhe$convergence == FALSE) {
  stop(paste("nlminb with gradient + hessian didn't converge properly, simu1 dataset, m =", m))
}
if (result_hjn$convergence == FALSE) {
  stop(paste("hjn didn't converge properly, simu1 dataset, m =", m))
}
if (result_marqLevAlg$convergence == FALSE) {
  stop(paste("marqLevAlg didn't converge properly, simu1 dataset, m =", m))
}
if (result_marqLevAlg_gr$convergence == FALSE) {
  stop(paste("marqLevAlg with gradient didn't converge properly, simu1 dataset, m =", m))
}
if (result_marqLevAlg_he$convergence == FALSE) {
  stop(paste("marqLevAlg with hessian didn't converge properly, simu1 dataset, m =", m))
}
if (result_marqLevAlg_grhe$convergence == FALSE) {
  stop(paste("marqLevAlg with gradient + hessian didn't converge properly, simu1 dataset, m =", m))
}
# if (result_ucminf$convergence == FALSE) {
#   stop(paste("ucminf didn't converge properly, simu1 dataset, m =", m))
# }
if (result_newuoa$convergence == FALSE) {
  stop(paste("newuoa didn't converge properly, simu1 dataset, m =", m))
}
if (result_BBoptim$convergence == FALSE) {
  stop(paste("BBoptim didn't converge properly, simu1 dataset, m =", m))
}
# if (result_BBoptim_gr$convergence == FALSE) {
#   stop(paste("BBoptim with gradient didn't converge properly, simu1 dataset, m =", m))
# }

nll_simu1 <- c("BFGS" = result_BFGS$nll,
...
# optimizers_estimates_simu1 <- do.call("list", setNames(lapply(paste0("par_", OPTIMIZERS_METHOD_UNDERSCORE),
...
#                                                                                                      indices = FALSE))),
#                                                       OPTIMIZERS_METHOD))
optimizers_estimates_simu1 <- list("BFGS" = result_BFGS[PARAMS_NAMES],
...
lambda_names <- paste0("lambda", 1:m)
...
lambdas_rowwise <- sapply(optimizers_estimates_simu1, "[[", "lambda")
gammas_rowwise <- sapply(optimizers_estimates_simu1, "[[", "gamma")
deltas_rowwise <- sapply(optimizers_estimates_simu1, "[[", "delta")
...
cv_nll <- sd(nll_simu1) / mean(nll_simu1)
if (any(cv_lambdas > 0.2) || any(cv_gammas > 0.2) || any(cv_nll > 0.2)) {
  stop("Simu1 data MLEs or nlls have a coefficient of variation above 20%")
}
  ...
  for (idx_counter_benchmark in 1:BENCHMARK_SAMPLES) {
    ...
    benchmark_model <- pois.HMM.generate.estimable.sample(ns = DATA_SIZE_SIMU1,
                                                          mod = list(m = m,
                                                                     ...
                                                                     gamma = simu1_true_gamma,
                                                                     delta = simu1_true_delta),
                                                          debug_message = "Simu1 benchmarking")
    ...
    #                          gamma_names)
    benchmark_optimizer_comparison_df_simu1 <- rbind(benchmark_optimizer_comparison_df_simu1,
...
if (BOOTSTRAP_SAMPLES != 0) {
  ...
  bootstrap_simu1_results <- foreach(idx_counter_bootstrap = 1:BOOTSTRAP_SAMPLES,
                                     ...
                                     .multicombine = TRUE) %dorng% {
                                       ...
                                       bootstrap_model <- pois.HMM.generate.estimable.sample(
                                         ...
                                         debug_message = "bootstrap simu1 parallel"
                                       )
  ...
  #   names(bootstrap_simu1) <- params_names_latex
  # } else {
  #   colnames(bootstrap_simu1) <- params_names_latex
  # }
  # q <- apply(bootstrap_simu1,
  #            2,
  #            quantile.colwise)
  # conf_int_simu1$Bootstrap.L <- q[1, ]
  # conf_int_simu1$Bootstrap.U <- q[2, ]
}
...
simu1_robustness <- lapply(X = true_params_simu1,
                           FUN = function(x, data_size, m) {
                             simu1_true_lambda <- x$lambda
                             simu1_true_gamma <- x$gamma
                             simu1_true_delta <- x$delta
                           ...
                           data_size = DATA_SIZE_SIMU1,
                           m = M_LIST_SIMU1)

# For each new simulated data set, we need the parameter possibilities
full_parameter_possibilities <- foreach(dataset_idx = seq_along(true_params_simu1),
                                        .packages = packages,
                                        .combine = rbind,
                                        .inorder = TRUE) %do% {
                                          
                                          simu_data <- simu1_robustness[[dataset_idx]]$data
                                          
                                          # Create parameters variations -------------------
                                          # lambda must be greater than 0. We make it greater than 0.5.
                                          min_lambda <- max(min(simu_data), 0.5)
                                          accuracy_lambdas <- seq(min_lambda, max(simu_data), by = 0.5)
                                          ...
                                          # parameter_possibilities <- do.call("CJ", append(params_lambda, params_gamma))
                                          ...
                                          parameter_possibilities <- merge(x = parameter_possibilities_lambda, y = parameter_possibilities_gamma)
                                          ...
                                          # full_parameter_possibilities <- rbindlist(lapply(1:length(simu1_robustness), function(x) cbind(parameter_possibilities, "dataset_idx" = x)))
                                          ...
                                          return(parameter_possibilities)
                                        }
...
# unlink("mylogsimu1.txt")
# file.create("mylogsimu1.txt")
...
results <- foreach(parameter_possibilities_row = iter(full_parameter_possibilities, by = "row"),
                   ...
                   .inorder = TRUE) %dorng% {
                     ...
                     accuracy_rates_names <- setNames(lapply(accuracy_rates_names, function(x) NA),
                                                      accuracy_rates_names)
                     accuracy_rates_simu1 <- do.call(data.frame, accuracy_rates_names)
                     accuracy_rates_simu1 <- accuracy_rates_simu1[, order(names(accuracy_rates_simu1))]
                     # accuracy_rates_simu1 <- cbind(m = rep(m, accuracy_samples_amount),
                     #                              # dataset_idx = parameter_possibilities_rows$dataset_idx,
                     #                              accuracy_rates_simu1)
                     accuracy_rates_simu1 <- cbind(m = m,
                                                   dataset_idx = parameter_possibilities_row$dataset_idx,
                                                   accuracy_rates_simu1,
                                                   idx = parameter_possibilities_row$idx)
                     
                     # accuracy_mles_simu1 <- data.frame()
                     
                     accuracy_dataset_idx <- parameter_possibilities_row$dataset_idx
                     accuracy_variables <- simu1_robustness[[accuracy_dataset_idx]]
                     ...
                     reference_lambda_accuracy <- accuracy_variables$lambda
                     ...
                     # Check convergence --------
                     accuracy_rates_simu1$BFGS_convergence_failure <- is.null(result_BFGS_accuracy) || !result_BFGS_accuracy$convergence
                     accuracy_rates_simu1$BFGS_gr_convergence_failure <- is.null(result_BFGS_gr_accuracy) || !result_BFGS_gr_accuracy$convergence
                     accuracy_rates_simu1$L_BFGS_B_convergence_failure <- is.null(result_L_BFGS_B_accuracy) || !result_L_BFGS_B_accuracy$convergence
                     accuracy_rates_simu1$L_BFGS_B_gr_convergence_failure <- is.null(result_L_BFGS_B_gr_accuracy) || !result_L_BFGS_B_gr_accuracy$convergence
                     accuracy_rates_simu1$CG_convergence_failure <- is.null(result_CG_accuracy) || !result_CG_accuracy$convergence
                     accuracy_rates_simu1$CG_gr_convergence_failure <- is.null(result_CG_gr_accuracy) || !result_CG_gr_accuracy$convergence
                     accuracy_rates_simu1$Nelder_Mead_convergence_failure <- is.null(result_Nelder_Mead_accuracy) || !result_Nelder_Mead_accuracy$convergence
                     accuracy_rates_simu1$nlm_convergence_failure <- is.null(result_nlm_accuracy) || !result_nlm_accuracy$convergence
                     accuracy_rates_simu1$nlm_gr_convergence_failure <- is.null(result_nlm_gr_accuracy) || !result_nlm_gr_accuracy$convergence
                     accuracy_rates_simu1$nlm_he_convergence_failure <- is.null(result_nlm_he_accuracy) || !result_nlm_he_accuracy$convergence
                     accuracy_rates_simu1$nlm_grhe_convergence_failure <- is.null(result_nlm_grhe_accuracy) || !result_nlm_grhe_accuracy$convergence
                     accuracy_rates_simu1$nlminb_convergence_failure <- is.null(result_nlminb_accuracy) || !result_nlminb_accuracy$convergence
                     accuracy_rates_simu1$nlminb_gr_convergence_failure <- is.null(result_nlminb_gr_accuracy) || !result_nlminb_gr_accuracy$convergence
                     accuracy_rates_simu1$nlminb_he_convergence_failure <- is.null(result_nlminb_he_accuracy) || !result_nlminb_he_accuracy$convergence
                     accuracy_rates_simu1$nlminb_grhe_convergence_failure <- is.null(result_nlminb_grhe_accuracy) || !result_nlminb_grhe_accuracy$convergence
                     accuracy_rates_simu1$hjn_convergence_failure <- is.null(result_hjn_accuracy) || !result_hjn_accuracy$convergence
                     accuracy_rates_simu1$marqLevAlg_convergence_failure <- is.null(result_marqLevAlg_accuracy) || !result_marqLevAlg_accuracy$convergence
                     accuracy_rates_simu1$marqLevAlg_gr_convergence_failure <- is.null(result_marqLevAlg_gr_accuracy) || !result_marqLevAlg_gr_accuracy$convergence
                     accuracy_rates_simu1$marqLevAlg_he_convergence_failure <- is.null(result_marqLevAlg_he_accuracy) || !result_marqLevAlg_he_accuracy$convergence
                     accuracy_rates_simu1$marqLevAlg_grhe_convergence_failure <- is.null(result_marqLevAlg_grhe_accuracy) || !result_marqLevAlg_grhe_accuracy$convergence
                     accuracy_rates_simu1$newuoa_convergence_failure <- is.null(result_newuoa_accuracy) || !result_newuoa_accuracy$convergence
                     accuracy_rates_simu1$BBoptim_convergence_failure <- is.null(result_BBoptim_accuracy) || !result_BBoptim_accuracy$convergence
                     
                     # Check nll ------------
                     accuracy_rates_simu1$BFGS_nll_found <- if (accuracy_rates_simu1$BFGS_convergence_failure) NA else abs(result_BFGS_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$BFGS_gr_nll_found <- if (accuracy_rates_simu1$BFGS_gr_convergence_failure) NA else abs(result_BFGS_gr_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$L_BFGS_B_nll_found <- if (accuracy_rates_simu1$L_BFGS_B_convergence_failure) NA else abs(result_L_BFGS_B_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$L_BFGS_B_gr_nll_found <- if (accuracy_rates_simu1$L_BFGS_B_gr_convergence_failure) NA else abs(result_L_BFGS_B_gr_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$CG_nll_found <- if (accuracy_rates_simu1$CG_convergence_failure) NA else abs(result_CG_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$CG_gr_nll_found <- if (accuracy_rates_simu1$CG_gr_convergence_failure) NA else abs(result_CG_gr_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$Nelder_Mead_nll_found <- if (accuracy_rates_simu1$Nelder_Mead_convergence_failure) NA else abs(result_Nelder_Mead_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$nlm_nll_found <- if (accuracy_rates_simu1$nlm_convergence_failure) NA else abs(result_nlm_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$nlm_gr_nll_found <- if (accuracy_rates_simu1$nlm_gr_convergence_failure) NA else abs(result_nlm_gr_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$nlm_he_nll_found <- if (accuracy_rates_simu1$nlm_he_convergence_failure) NA else abs(result_nlm_he_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$nlm_grhe_nll_found <- if (accuracy_rates_simu1$nlm_grhe_convergence_failure) NA else abs(result_nlm_grhe_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$nlminb_nll_found <- if (accuracy_rates_simu1$nlminb_convergence_failure) NA else abs(result_nlminb_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$nlminb_gr_nll_found <- if (accuracy_rates_simu1$nlminb_gr_convergence_failure) NA else abs(result_nlminb_gr_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$nlminb_he_nll_found <- if (accuracy_rates_simu1$nlminb_he_convergence_failure) NA else abs(result_nlminb_he_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$nlminb_grhe_nll_found <- if (accuracy_rates_simu1$nlminb_grhe_convergence_failure) NA else abs(result_nlminb_grhe_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$hjn_nll_found <- if (accuracy_rates_simu1$hjn_convergence_failure) NA else abs(result_hjn_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$marqLevAlg_nll_found <- if (accuracy_rates_simu1$marqLevAlg_convergence_failure) NA else abs(result_marqLevAlg_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$marqLevAlg_gr_nll_found <- if (accuracy_rates_simu1$marqLevAlg_gr_convergence_failure) NA else abs(result_marqLevAlg_gr_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$marqLevAlg_he_nll_found <- if (accuracy_rates_simu1$marqLevAlg_he_convergence_failure) NA else abs(result_marqLevAlg_he_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$marqLevAlg_grhe_nll_found <- if (accuracy_rates_simu1$marqLevAlg_grhe_convergence_failure) NA else abs(result_marqLevAlg_grhe_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$newuoa_nll_found <- if (accuracy_rates_simu1$newuoa_convergence_failure) NA else abs(result_newuoa_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     accuracy_rates_simu1$BBoptim_nll_found <- if (accuracy_rates_simu1$BBoptim_convergence_failure) NA else abs(result_BBoptim_accuracy$nll / reference_nll_accuracy - 1) <= NLL_THRESHOLD
                     
                     
                     # # Record estimates & results ---------
                     # # par_BFGS_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "BFGS_convergence_failure"] == FALSE) pois.HMM.pw2pn(m, result_BFGS_accuracy$par) else NA
                     # # par_L_BFGS_B_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "L_BFGS_B_convergence_failure"] == FALSE) pois.HMM.pw2pn(m, result_L_BFGS_B_accuracy$par) else NA
                     # # par_CG_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "CG_convergence_failure"] == FALSE) pois.HMM.pw2pn(m, result_CG_accuracy$par) else NA
                     # # par_Nelder_Mead_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "Nelder_Mead_convergence_failure"] == FALSE) pois.HMM.pw2pn(m, result_Nelder_Mead_accuracy$par) else NA
                     # # par_nlm_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "nlm_convergence_failure"] == FALSE) pois.HMM.pw2pn(m, result_nlm_accuracy$estimate) else NA
                     # # par_nlminb_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "nlminb_convergence_failure"] == FALSE) pois.HMM.pw2pn(m, result_nlminb_accuracy$par) else NA
                     # # par_hjn_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "hjn_convergence_failure"] == FALSE) pois.HMM.pw2pn(m, result_hjn_accuracy$par) else NA
                     # # par_marqLevAlg_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "marqLevAlg_convergence_failure"] == FALSE) pois.HMM.pw2pn(m, result_marqLevAlg_accuracy$b) else NA
                     # # # par_ucminf_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "ucminf_convergence_failure"] == FALSE) pois.HMM.pw2pn(m, result_ucminf_accuracy$par) else NA
                     # # par_newuoa_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "newuoa_convergence_failure"] == FALSE) pois.HMM.pw2pn(m, result_newuoa_accuracy$par) else NA
                     # # par_BBoptim_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "BBoptim_convergence_failure"] == FALSE) pois.HMM.pw2pn(m, result_BBoptim_accuracy$par) else NA
                     # 
                     ...
                     # estimates_BFGS_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "BFGS_convergence_failure"]) NA else unlist(result_BFGS_accuracy[PARAMS_NAMES])
                     # estimates_BFGS_gr_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "BFGS_gr_convergence_failure"]) NA else unlist(result_BFGS_gr_accuracy[PARAMS_NAMES])
                     # estimates_L_BFGS_B_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "L_BFGS_B_convergence_failure"]) NA else unlist(result_L_BFGS_B_accuracy[PARAMS_NAMES])
                     # estimates_L_BFGS_B_gr_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "L_BFGS_B_gr_convergence_failure"]) NA else unlist(result_L_BFGS_B_gr_accuracy[PARAMS_NAMES])
                     # estimates_CG_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "CG_convergence_failure"]) NA else unlist(result_CG_accuracy[PARAMS_NAMES])
                     # estimates_CG_gr_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "CG_gr_convergence_failure"]) NA else unlist(result_CG_gr_accuracy[PARAMS_NAMES])
                     # estimates_Nelder_Mead_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "Nelder_Mead_convergence_failure"]) NA else unlist(result_Nelder_Mead_accuracy[PARAMS_NAMES])
                     # estimates_nlm_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "nlm_convergence_failure"]) NA else unlist(result_nlm_accuracy[PARAMS_NAMES])
                     # estimates_nlm_gr_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "nlm_gr_convergence_failure"]) NA else unlist(result_nlm_gr_accuracy[PARAMS_NAMES])
                     # estimates_nlm_he_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "nlm_he_convergence_failure"]) NA else unlist(result_nlm_he_accuracy[PARAMS_NAMES])
                     # estimates_nlm_grhe_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "nlm_grhe_convergence_failure"]) NA else unlist(result_nlm_grhe_accuracy[PARAMS_NAMES])
                     # estimates_nlminb_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "nlminb_convergence_failure"]) NA else unlist(result_nlminb_accuracy[PARAMS_NAMES])
                     # estimates_nlminb_gr_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "nlminb_gr_convergence_failure"]) NA else unlist(result_nlminb_gr_accuracy[PARAMS_NAMES])
                     # estimates_nlminb_he_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "nlminb_he_convergence_failure"]) NA else unlist(result_nlminb_he_accuracy[PARAMS_NAMES])
                     # estimates_nlminb_grhe_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "nlminb_grhe_convergence_failure"]) NA else unlist(result_nlminb_grhe_accuracy[PARAMS_NAMES])
                     # estimates_hjn_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "hjn_convergence_failure"]) NA else unlist(result_hjn_accuracy[PARAMS_NAMES])
                     # estimates_marqLevAlg_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "marqLevAlg_convergence_failure"]) NA else unlist(result_marqLevAlg_accuracy[PARAMS_NAMES])
                     # estimates_marqLevAlg_gr_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "marqLevAlg_gr_convergence_failure"]) NA else unlist(result_marqLevAlg_gr_accuracy[PARAMS_NAMES])
                     # estimates_marqLevAlg_he_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "marqLevAlg_he_convergence_failure"]) NA else unlist(result_marqLevAlg_he_accuracy[PARAMS_NAMES])
                     # estimates_marqLevAlg_grhe_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "marqLevAlg_grhe_convergence_failure"]) NA else unlist(result_marqLevAlg_grhe_accuracy[PARAMS_NAMES])
                     # estimates_newuoa_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "newuoa_convergence_failure"]) NA else unlist(result_newuoa_accuracy[PARAMS_NAMES])
                     # estimates_BBoptim_accuracy <- if (accuracy_rates_simu1[idx_counter_accuracy, "BBoptim_convergence_failure"]) NA else unlist(result_BBoptim_accuracy[PARAMS_NAMES])
                     # 
                     # rownames_mles_accuracy <- c(paste0("lambda", 1:m), paste0("gamma", 1:(m ^ 2)), paste0("delta", 1:m))
                     # accuracy_mles_simu1 <- rbind(accuracy_mles_simu1, data.frame("m" = m,
                     ...
                     # lock_log <- lock(lockfile_path)
                     # # if (Sys.info()[['sysname']] == "Windows") {
                     # #   shell(paste0("sed -ri 's/^(", parameter_possibilities_row$idx ,")$/\\1 END/' mylogsimu1.txt"))
                     # # } else {
                     # #   system(paste0("sed -ri 's/(", parameter_possibilities_row$idx ,")$/\\1 END/' mylogsimu1.txt"))
                     # # }
                     # cat(paste0(parameter_possibilities_row$idx, "\n"), sep = "", file="mylogsimu1.txt", append=TRUE)
                     # 
                     # if (Sys.info()[['sysname']] == "Windows") {
                     #   # current_amount_done <- as.numeric(shell("grep -c 'END' mylogsimu1.txt", intern = TRUE, mustWork = NA))
                     #   current_amount_done <- as.numeric(shell("wc -l mylogsimu1.txt | cut -f 1 -d ' '", intern = TRUE, mustWork = NA))
                     # } else {
                     #   # current_amount_done <- as.numeric(system("wc -l mylogsimu1.txt | cut -f 1 -d ' '", intern = TRUE))
                     #   current_amount_done <- as.numeric(system("wc -l mylogsimu1.txt | cut -f 1 -d ' '", intern = TRUE))
                     # }
                     # unlock(lock_log)
                     ...
                     # return(list(accuracy_rates_simu1 = accuracy_rates_simu1, accuracy_mles_simu1 = accuracy_mles_simu1))
                     # return(list(accuracy_rates_simu1 = accuracy_rates_simu1))
                     return(accuracy_rates_simu1)
                   }
# stopCluster(cl)
# stopImplicitCluster()
future:::ClusterRegistry("stop")
accuracy_rates_simu1 <- results
# accuracy_mles_simu1 <- results$accuracy_mles_simu1

# colnames(accuracy_mles_simu1) <- params_names_latex
# 
# for (param_name in c(paste0("lambda", 1:m), paste0("gamma", 1:(m ^ 2)))) {
#   sapply(accuracy_mles_simu1[accuracy_mles_simu1$par_name == param_name, 5:14], sd, na.rm = TRUE)
# }
# q <- apply(accuracy_rates_simu1,
#            2,
#            quantile.colwise)
...
# estim_benchmarks_df_simu1$m <- factor(estim_benchmarks_df_simu1$m,
#                                      levels = M_LIST_SIMU1)

# Fixes -------------------------
if (BOOTSTRAP_SAMPLES != 0) {
  # Reorder the TPM row-wise instead of column-wise
  # Rename parameters in bootstrap_simu1_results to TeX greek letters
  ...
  # indices <- (length(bootstrap_simu1_results$m) + 1):(length(bootstrap_simu1_results$m) + len_par)
  bootstrap_simu1_results$all_natural_parameters$Parameter <- params_names_latex
  
  all_natural_parameters <- bootstrap_simu1_results$all_natural_parameters
  ...
  bootstrap_simu1_results$all_natural_parameters <- all_natural_parameters
}


notif(title = "Simu1 Robustness FINI",
      msg = "fin du fichier")
sink()