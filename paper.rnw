...
<<import-files, echo = FALSE, cache = FALSE, include=FALSE>>=
source("./code/main.R")
read_chunk('./functions/utils.R')
# read_chunk('./functions/utils.R', labels = "setup",
#            from = "# Setup", to = "# Compute log-forward",
#            to.offset = -2)
# read_chunk('./functions/utils.R', labels = "log-forward",
#            from = "# Compute log-forward", to = "# Compute log-backwards",
#            to.offset = -1)
# read_chunk('./functions/utils.R', labels = "log-backward",
#            from = "# Compute log-backwards", to = "# Compute conditional",
#            to.offset = -1)
# read_chunk('./functions/utils.R', labels = "local decoding",
#            from = "# Compute conditional", to = "# Compute forecast",
#            to.offset = -1)
# read_chunk('./functions/utils.R', labels = "h-step",
#            from = "# Compute forecast", to = "# Output as list",
#            to.offset = -1)
@
...
In a second part, we compare different optimizers such as {\tt{nlminb}}, and minimize the negative log-likelihood directly on different datasets: a small one (\Sexpr{DATA_SIZE_LAMB} data points) from \citet{leroux}, a medium sized simulated one (\Sexpr{DATA_SIZE_SIMU} data points), and a large one (\Sexpr{DATA_SIZE_HOSP} data points) from a hospital.
% 
% Why we did it
...
<<tinnitus-table, results = 'asis', echo = FALSE>>=
...
<<profile_plot, fig.cap = 'Profile likelihood'>>=
...
%GEIR: REFORMULATE? In addition, this method can sometimes give NA values, usually when a ML estimate is close to a boundary, but not only. For example, the first working parameter \texttt{log\_lambda} is pretty low ($\Sexpr{round(sdreport(obj_tmb)$par.fixed[m ^ 2], 2)}$). It becomes too difficult to profile the likelihood so the function fails to provide a meaningful confidence interval.\\
...
As preliminary reliability check of our IT infrastructure and setup, we timed the fitting of our 2-state HMM to the TYT data with the help of the {\tt{microbenchmark}} package. For this data set, all five approached converged to the same maximum. \autoref{table:speed-consistency-tinn} shows the resulting average time required for the parameter estimation and the number of iterations needed by each approach, measured over $\Sexpr{CONSISTENCY_BENCHMARK_TINN}$ replications. The results show that the use of {\tt{TMB}} significantly accelerates parameter estimation in comparison with $DM$. The strongest acceleration is achieved by $TMB_G$, underlining the benefit of using the gradient provided by {\tt{TMB}}. Moreover, $TMB_{GH}$ requires less iterations than the other approaches. However, the approximation of the Hessian seems to increase the computational burden.\\ [1ex]     
...
<<tinn-estimates, results='asis', echo = FALSE>>=
idx <- 1
...
<<tmb-acceleration-consistency-tinn-table, results='asis', echo = FALSE>>=
library(xtable)
incr_consist_tinn <- data.frame("text" = character(),
                                "DM" = character(),
                                "TMB" = character(),
                                "TMB_G" = character(),
                                "TMB_H" = character(),
                                "TMB_GH" = character())
for (idx in seq(1, 2 * length(M_LIST_TINN), by = 2)) {
  m <- M_LIST_TINN[idx]
  incr_consist_tinn[idx, "text"] <- "Time"
  incr_consist_tinn[idx + 1, "text"] <- "Iterations"
  boxplot_data <- consistency_estim_benchmarks_df_tinn[consistency_estim_benchmarks_df_tinn$m == m, ]
  ...
  for (proc in PROCEDURES) {
    ...
    ci <- c(avg - qnorm(0.975) * sd(time)/sqrt(CONSISTENCY_BENCHMARK_TINN),
            avg + qnorm(0.975) * sd(time)/sqrt(CONSISTENCY_BENCHMARK_TINN))
    iter <- boxplot_data[boxplot_data$procedure == proc, "iterations"][1]
    # \makecell{AA \\ BB}
    # incr_consist_tinn[idx, proc] <- paste0(
    #   "\\makecell{",
    #   signif(avg,3), " (", signif(ci[1],3), ", ", signif(ci[2],3), ") ",
    #   "\\\\ ",
    #   iter,
    #   " iterations",
    #   "}")
    incr_consist_tinn[idx, proc] <- paste0(signif(avg,3),
                                           " (",
                                           signif(ci[1],3),
                                           ", ",
                                           signif(ci[2],3),
                                           ")")
    incr_consist_tinn[idx + 1, proc] <- iter
  }
}
mt <- M_LIST_TINN[1]
names(incr_consist_tinn) <- c("",
                              "\\textit{${DM}$}",
                              "\\textit{${TMB}$}",
                              "\\textit{${TMB_G}$}",
                              "\\textit{${TMB_H}$}",
                              "\\textit{${TMB_{GH}}$}")
table <- xtable(incr_consist_tinn,
                # display = c("d", "d", rep("s", 4)),
                # align = paste0("lr", do.call(paste0, as.list(rep("p{2cm}", 5)))),
                caption = paste0("Average duration (in milliseconds) together with 95\\% CI and number of iterations required for fitting a 2-state Poisson HMM to the TYT data. The CIs are of Wald-type and base on the standard error of the mean derived from ", CONSISTENCY_BENCHMARK_TINN, " replications."),
...
%paste0("Average duration (in milliseconds) together with 95\\% CI and number of iterations required for fitting a 2-state Poisson HMM to the TYT data. The CIs are of Wald-type and base on the the standard error of the mean derived from", $\Sexpr{CONSISTENCY_BENCHMARK_TINN}$, "replications.")
...
for (idx in 1:length(M_LIST_TINN)) {
  m <- M_LIST_TINN[idx]
  # incr_tinn[idx, "m"] <- m
  incr_tinn[idx, "text"] <- "Time"
  incr_tinn[idx + 1, "text"] <- "Iterations"
  boxplot_data <- estim_benchmarks_df_tinn[estim_benchmarks_df_tinn$m == m, ]
  ...
  for (proc in PROCEDURES[-1]) {
    ...
    incr_tinn[idx, proc] <- paste0(signif(avg_ratio,3), " (", signif(ci_ratio[1],3), ", ", signif(ci_ratio[2],3), ")")
    
    iter <- boxplot_data[boxplot_data$procedure == proc, "iterations"]
    avg_iter <- mean(iter)
    ci_iter <- quantile.colwise(iter)
    incr_tinn[idx + 1, proc] <- paste0(signif(avg_iter,3), " (", signif(ci_iter[1],3), ", ", signif(ci_iter[2],3), ")")
  }
}
mt <- M_LIST_TINN[1]
names(incr_tinn) <- c("",
                      ...
                      "\\textit{${TMB_H}$}",
                      "\\textit{${TMB_{GH}}$}")
table <- xtable(incr_tinn,
                # display = c("d", "d", rep("s", 4)),
                caption = paste0("Acceleration ratios together with 95\\% CIs when using {\\tt TMB} in a bootstrap setting with ", BENCHMARK_SAMPLES, " replications"),
...
As last check of the acceleration obtained by {\tt{TMB}}, we also timed the computation of log-likelihood alone. To do so, we used the same parametric bootstrap-type setting as the one described above with $\Sexpr{BENCHMARK_SAMPLES}$ replications. The acceleration factor of {\tt{TMB}} compared to conventional {\tt R} code was estimated as 12.5 (8.12, 16.2).
...
<<tmb-acceleration-tinn-table-nll, results='asis', echo = FALSE>>=
incr_mllk_time_tinn <- data.frame(m = integer(),
                                  "TMB" = character(),
                                  "TMB_G" = character(),
                                  "TMB_H" = character(),
                                  "TMB_GH" = character())
for (idx in 1:length(M_LIST_TINN)) {
  m <- M_LIST_TINN[idx]
  incr_mllk_time_tinn[idx, "m"] <- m
  boxplot_data <- mllk_times_df_tinn[mllk_times_df_tinn$m == m, ]
  ...
  for (proc in PROCEDURES[-1]) {
    ...
    incr_mllk_time_tinn[idx, proc] <- paste0(signif(avg,3), " (", signif(ci[1],3), ", ", signif(ci[2],3), ")")
  }
}
mt <- M_LIST_TINN[1]
names(incr_mllk_time_tinn) <- c("m",
                              "\\textit{${TMB}$}",
                              "\\textit{${TMB_G}$}",
                              "\\textit{${TMB_H}$}",
                              "\\textit{${TMB_{GH}}$}")
table <- xtable(incr_mllk_time_tinn,
                # display = c("d", "d", rep("f", 4)),
                caption = paste0("Acceleration when using TMB for m state Poisson HMM negative log-likelihood calculation, estimated on the tinnitus dataset. Each procedure was repeated and timed once on ", BENCHMARK_SAMPLES, " sample datasets. With ", mt, " hidden states, $TMB_G$ accelerated the estimation by an average factor of ", substr(incr_mllk_time_tinn[1, 3], 1, 2), ", with a 95\\% quantile confidence interval of ", substr(incr_mllk_time_tinn[1, 3], 4, 11)),
...
BOOTSTRAP\_SAMPLES=\Sexpr{BOOTSTRAP_SAMPLES} replications for bootstrap CIs\\
COVERAGE\_SAMPLES=\Sexpr{COVERAGE_SAMPLES} replications for coverage CIs\\
BENCHMARK\_SAMPLES=\Sexpr{BENCHMARK_SAMPLES} replications for the benchmarking.\\
...
JAN: I used only the values that were not missing. For the stationary dist, it is too complicated to compute the profile CI, so I did not do it.



<<tinn-estimates-table, results = 'asis', echo = FALSE>>=
addtorow <- list()
addtorow$pos <- list(- 1)
addtorow$command <- paste0('&',
                           paste0('& \\multicolumn{2}{c}{',
                                  c('{\\tt TMB} CI', 'Profile CI', 'Bootstrap CI'),
                                  '}',
                                  collapse=''),
                           '& \\multicolumn{3}{c}{Coverage prob.}',
                           '\\\\')

table <- xtable(conf_int_tinn[, - 1],
                ...
                align = rep("c", ncol(conf_int_tinn[, - 1]) + 1))

names(table) <- c("Parameter", "Estimate", rep(c("L.", "U."), 3),
                  '{\\tt TMB}', 'Profile', 'Bootstrap')
print(table,
...
($\Sexpr{M_LIST_LAMB}$) hidden states were specified in the HMM estimated on the lamb dataset,
...
for (idx in 1:length(M_LIST_LAMB)) {
  m <- M_LIST_LAMB[idx]
  incr_lamb[idx, "m"] <- m
  boxplot_data <- estim_benchmarks_df_lamb[estim_benchmarks_df_lamb$m == m, ]
  ...
  for (proc in PROCEDURES[-1]) {
    ...
    avg <- mean(ratio)
    ci <- quantile.colwise(ratio)
    incr_lamb[idx, proc] <- paste0(signif(avg,3), " (", signif(ci[1],3), ", ", signif(ci[2],3), ")")
  }
}
...
table <- xtable(incr_lamb,
                # display = c("d", "d", rep("s", 4)),
                caption = paste0("Acceleration when using TMB for m state Poisson HMM parameter estimation, estimated on the lamb dataset. Each procedure was repeated and timed once on ", BENCHMARK_SAMPLES, " sample datasets. With ", mt, " hidden states, $TMB_G$ accelerated the estimation by an average factor of ", substr(incr_lamb[1, 3], 1, 2), ", with a 95\\% quantile confidence interval of ", substr(incr_lamb[1, 3], 4, 11)),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 ($\Sexpr{M_LIST_SIMU}$) for the simulated dataset,



The second dataset is a sequence of random numbers generated by a $m$-state Poisson HMM.
We used the code below to generate that simulated dataset, with $m=\Sexpr{M_LIST_SIMU}$.
...
<<tmb-acceleration-simu-table, results='asis', echo = FALSE>>=
library(xtable)
incr_simu <- data.frame(m = integer(),
                        "TMB" = character(),
                        "TMB_G" = character(),
                        "TMB_H" = character(),
                        "TMB_GH" = character())
for (idx in 1:length(M_LIST_SIMU)) {
  m <- M_LIST_SIMU[idx]
  incr_simu[idx, "m"] <- m
  boxplot_data <- estim_benchmarks_df_simu[estim_benchmarks_df_simu$m == m, ]
  ...
  for (proc in PROCEDURES[-1]) {
    ...
    avg <- mean(ratio)
    ci <- quantile.colwise(ratio)
    incr_simu[idx, proc] <- paste0(signif(avg,3), " (", signif(ci[1],3), ", ", signif(ci[2],3), ")")
  }
}
mt <- M_LIST_SIMU[1]
names(incr_simu) <- c("m",
                      ...
                      "\\textit{${TMB_H}$}",
                      "\\textit{${TMB_{GH}}$}")
table <- xtable(incr_simu,
                # display = c("d", "d", rep("s", 4)),
                caption = paste0("Acceleration when using TMB for m state Poisson HMM parameter estimation, estimated on the simulated dataset. Each procedure was repeated and timed once on ", BENCHMARK_SAMPLES, " sample datasets. With ", mt, " hidden states, $TMB_G$ accelerated the estimation by an average factor of ", substr(incr_simu[1, 3], 1, 2), ", with a 95\\% quantile confidence interval of ", substr(incr_simu[1, 3], 4, 11)),
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
 ...
 and ($\Sexpr{M_LIST_HOSP}$) for the hospital dataset) to compare speeds.


% 
% We timed $m$ states HMM parameter estimations using different parameters on a dataset provided by a hospital from Assistance Publique – Hôpitaux de Paris, a french hospital trust, with $m = \Sexpr{M_LIST_HOSP}$.
...
% That data contains \Sexpr{DATA_SIZE_HOSP} data points.\\
...
for (idx in 1:length(M_LIST_HOSP)) {
  ...
  for (proc in PROCEDURES[-1]) {
    ...
    avg <- mean(ratio)
    ci <- quantile.colwise(ratio)
    incr_hosp[idx, proc] <- paste0(round(avg), " (", round(ci[1]), ", ", round(ci[2]), ")")
  }
}
mt <- M_LIST_HOSP[1]
names(incr_hosp) <- c("m",
...
table <- xtable(incr_hosp,
                # display = c("d", "d", rep("s", 4)),
                caption = paste0("Acceleration when using TMB for m state Poisson HMM parameter estimation, estimated on the hospital dataset. Each procedure was repeated and timed once on ", BENCHMARK_SAMPLES, " bootstrap sample datasets. With ", mt, " hidden states, $TMB_G$ accelerated the estimation by an average factor of ", substr(incr_hosp[1, 3], 1, 2), ", with a 95\\% quantile bootstrap confidence interval of ", substr(incr_hosp[1, 3], 4, 11)),
...
@
% 
% Each box of the boxplots summarizes $\Sexpr{BENCHMARK_SAMPLES}$ data points but shows little variation.
...
% For example, the average 4 state Poisson HMM estimation speed when providing {\tt{TMB}}'s exact gradient to {\tt{nlminb}} and not the hessian (TMB3) is $\Sexpr{round(incr_hosp[incr_hosp$m == 4, "TMB3"])}$\% faster than without making use of {\tt{TMB}} (DM).
% 
...
% @
% We find a p-value approximated to $\Sexpr{pvalue}$, so the test shows that the difference between the average amount of patient arrival between 6 and 6:59am and the one between 10 and 10:59am is statistically significant.
...
%   theme_Publication()
% @
% States (1, 2, 3, 4) have Poisson means (\Sexpr{round(tmb4$lambda, 3)}) respectively.
% 
...
addtorow$command <- paste0('&&',
                           paste0('& \\multicolumn{2}{c}{',
                                  c('Profile', 'Bootstrap', 'TMB'),
                                  '}',
                                  collapse=''),
                           '& \\multicolumn{3}{c}{Coverage}',
                           '\\\\')

colnames(conf_int_lamb) <- c("m", "Parameter", "Estimate", rep(c("Lower", "Upper"), 3),
                             'Profile', 'Bootstrap', 'TMB')
print(xtable(conf_int_lamb[conf_int_lamb$m <= 2, ],
             ...
             align = rep("c", dim(conf_int_lamb)[2] + 1)),
      include.rownames = FALSE,
      # scalebox = '0.8',
      sanitize.text.function = identity,
      add.to.row = addtorow)
...
colnames(conf_int_simu) <- c("m", "Parameter", "True value", "Estimate", rep(c("Lower", "Upper"), 3),
                             'Profile', 'Bootstrap', 'TMB')
print(xtable(conf_int_simu[conf_int_simu$m <= 3, ],
             caption = "Estimates and standard errors on the simulated dataset",
             label = "table:simul_estimates_std_errors",
             align = rep("c", dim(conf_int_simu)[2] + 1)),
      include.rownames = FALSE,
      scalebox = '0.95',
      sanitize.text.function = identity,
      add.to.row = addtorow)
...
colnames(conf_int_hosp) <- c("m", "Parameter", "Estimate", rep(c("Lower", "Upper"), 3))
                             # 'Profile', 'Bootstrap', 'TMB')
print(xtable(conf_int_hosp,
             caption = "Estimates and standard errors on the hospital dataset",
             label = "table:hosp_estimates_std_errors",
             align = rep("c", dim(conf_int_hosp)[2] + 1)),
      include.rownames = FALSE,
      # scalebox = '0.8',
      sanitize.text.function = identity,
      add.to.row = addtorow)
