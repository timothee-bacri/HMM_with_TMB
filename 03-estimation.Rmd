...
\citet{zucchini} shows that calculating the stationary distribution can be achieved by solving \autoref{eq:stat-dist} for $\b{\delta}$, where $\b{I}_m$ is the $m*m$ identity matrix, $\b{U}$ is a $m*m$ matrix of ones, and $\b{1}$ is a row vector of ones.
\[
...
## Negative log-likelihood<br> in `TMB`
