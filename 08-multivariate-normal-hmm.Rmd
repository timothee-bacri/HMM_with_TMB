...
In turn, these cause the density function to change from `dpois` to `dmvnorm` (and from `rpois` to `rmvnorm` for simulations) from the `mvtnorm` [@R-mvtnorm] package.
...
We generate 1000 data with the function below (similarly to the Poisson case).
