...
for (idx in 1:length(M_LIST_TINN)) {
  ...
  if (CONSISTENCY_BENCHMARK_TINN != 0) {
    ...
    temp <- microbenchmark("DM" = nlminb(parvect_init,
                                         pois.HMM.mllk,
                                         x_alias = tinn_data,
                                         m_alias = m)$convergence==0,
                           "TMB" = nlminb(obj_init$par,
                                          obj_init$fn)$convergence==0,
                           "TMB_G" = nlminb(obj_init$par,
                                            obj_init$fn,
                                            gradient = obj_init$gr)$convergence==0,
                           "TMB_H" = nlminb(obj_init$par,
                                            obj_init$fn,
                                            hessian = obj_init$he)$convergence==0,
                           "TMB_GH" = nlminb(obj_init$par,
                                             obj_init$fn,
                                             gradient = obj_init$gr,
                                             hessian = obj_init$he)$convergence==0,
                           times = CONSISTENCY_BENCHMARK_TINN,
                           setup = obj_init <<- MakeADFun(TMB_data,
                                                          working_params_init,
                                                          DLL = "poi_hmm",
                                                          silent = TRUE),
                           check = "equal")
    
    iterations <- c(nlminb(parvect_init,
                           pois.HMM.mllk,
                           x_alias = tinn_data,
                           m_alias = m)$iterations,
                    nlminb(obj_init$par,
                           obj_init$fn)$iteration,
                    nlminb(obj_init$par,
                           obj_init$fn,
                           gradient = obj_init$gr)$iteration,
                    nlminb(obj_init$par,
                           obj_init$fn,
                           hessian = obj_init$he)$iteration,
                    nlminb(obj_init$par,
                           obj_init$fn,
                           gradient = obj_init$gr,
                           hessian = obj_init$he)$iteration)
