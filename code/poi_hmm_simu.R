...
for (idx in 1:length(M_LIST_SIMU)) {
  ...
  lambda_init <- seq(1, 20, length.out = m)
  ...
  if (dm$convergence != 0) {
    stop(paste("dm didn't converge properly, simu dataset, m =", m))
  }
  if (tmb$convergence != 0) {
    stop(paste("tmb didn't converge properly, simu dataset, m =", m))
  }
  if (tmb_g$convergence != 0) {
    stop(paste("tmb_g didn't converge properly, simu dataset, m =", m))
  }
  if (tmb_h$convergence != 0) {
    stop(paste("tmb_h didn't converge properly, simu dataset, m =", m))
  }
  if (tmb_gh$convergence != 0) {
    stop(paste("tmb_gh didn't converge properly, simu dataset, m =", m))
  }
  ...
  mllk_values_simu[indexes, ] <- cbind(m = rep(m, 5),
  ...
  indexes <- (length(conf_int_simu$m) + 1):(length(conf_int_simu$m) + len_par)
  conf_int_simu[indexes, "m"] <- m
  conf_int_simu[indexes, "Parameter"] <- params_names_latex
  # Reminder, PARAMS_NAMES contains c("lambda", "gamma", "delta")
  conf_int_simu[indexes, "Estimate"] <- unlist(tmb_CI[PARAMS_NAMES])
  conf_int_simu[indexes, "True.value"] <- as.numeric(c(lambda_init, gamma_init, delta_init))
  ...
  temp <- microbenchmark("DM" = nlminb(parvect_init, pois.HMM.mllk, x_alias = simu_data, m_alias = m)$convergence==0,
                         ...
                         times = CONSISTENCY_BENCHMARK_SIMU,
                         control = list(order = "block", warmup = CONSISTENCY_BENCHMARK_SIMU / 4),
                         setup = obj_init <<- MakeADFun(TMB_data, working_params_init, DLL = "poi_hmm", silent = TRUE),
                         check = "equal")
  iterations <- c(nlminb(parvect_init, pois.HMM.mllk, x_alias = simu_data, m_alias = m)$iterations,
  ...
  consistency_estim_benchmarks_df_simu <- rbind(consistency_estim_benchmarks_df_simu,
                                                data.frame(time = c(timeDM, timeTMB, timeTMB_G, timeTMB_H, timeTMB_GH),
                                                           m = rep(m, length(PROCEDURES) * CONSISTENCY_BENCHMARK_SIMU),
                                                           procedure = rep(PROCEDURES, each = CONSISTENCY_BENCHMARK_SIMU),
                                                           iterations = rep(iterations, each = CONSISTENCY_BENCHMARK_SIMU)))
  ...
  for (idx_counter in 1:BENCHMARK_SAMPLES) {
    ...
    benchmark_model <- pois.HMM.generate_estimable_sample(ns = DATA_SIZE_SIMU,
                                                          mod = list(m = m,
    ...
    timeTMB_H <- times[temp$expr == "TMB_H"]
    timeTMB_GH <- times[temp$expr == "TMB_GH"]
    estim_benchmarks_df_simu <- rbind(estim_benchmarks_df_simu,
                                      data.frame(time = c(timeDM, timeTMB, timeTMB_G, timeTMB_H, timeTMB_GH),
    ...
    temp <- microbenchmark("DM" = pois.HMM.mllk(parvect_mllk, simu_data, m),
    ...
    timeTMB_H <- times[temp$expr == "TMB_H"]
    timeTMB_GH <- times[temp$expr == "TMB_GH"]
    mllk_times_df_simu <- rbind(mllk_times_df_simu,
                                data.frame(time = c(timeDM, timeTMB, timeTMB_G, timeTMB_H, timeTMB_GH),
                                           m = rep(m, length(PROCEDURES)),
                                           procedure = PROCEDURES,
                                           dataset_number = rep(idx_counter, length(PROCEDURES))))
    ...
    method_comparison_df_simu <- rbind(method_comparison_df_simu,
  ...
  # Lambda (m values)
  conf_int_simu$Profile.L[which(conf_int_simu$m == m)][1:m] <- exp(working_conf_int$lower[1:m])
  ...
  if (!anyNA(working_conf_int$lower[(m + 1):(m ^ 2)])) {
    ...
    conf_int_simu$Profile.L[which(conf_int_simu$m == m)][(m + 1):(m ^ 2 + m)] <- natural_gamma
  }
  # Lambda (m values)
  conf_int_simu$Profile.U[which(conf_int_simu$m == m)][1:m] <- exp(working_conf_int$upper[1:m])
  # Gamma (m^2-m working parameters, m^2 natural ones)
  if (!anyNA(working_conf_int$upper[(m + 1):(m ^ 2)])) {
    natural_gamma <- as.numeric(Gamma_w2n(m, working_conf_int$upper[(m + 1):(m ^ 2)]))
    conf_int_simu$Profile.U[which(conf_int_simu$m == m)][(m + 1):(m ^ 2 + m)] <- natural_gamma
  }
  
  # Bootstrap ---------------------------
  bootstrap_simu <- data.frame()
  lambda <- tmb_CI$lambda
  gamma <- tmb_CI$gamma
  delta <- tmb_CI$delta
  for (idx_sample in 1:BOOTSTRAP_SAMPLES) {
    temp <- pois.HMM.generate_estimable_sample(ns = DATA_SIZE_SIMU,
                                               mod = list(m = m,
                                                          lambda = tmb_CI$lambda,
                                                          gamma = tmb_CI$gamma,
                                                          delta = tmb_CI$delta),
    ...
    bootstrap_simu[idx_sample, 1:len_par] <- natural_parameters
  }
  colnames(bootstrap_simu) <- params_names_latex
  q <- apply(bootstrap_simu, 2, quantile.colwise)
  conf_int_simu$Bootstrap.L[which(conf_int_simu$m == m)] <- q[1, ]
  conf_int_simu$Bootstrap.U[which(conf_int_simu$m == m)] <- q[2, ]
  
  # TMB confidence intervals --------------
  conf_int_simu$TMB.L[which(conf_int_simu$m == m)] <- c(tmb_gh$lambda - q95_norm * tmb_gh$lambda_std_error,
                                                        tmb_gh$gamma - q95_norm * tmb_gh$gamma_std_error,
                                                        tmb_gh$delta - q95_norm * tmb_gh$delta_std_error)
  conf_int_simu$TMB.U[which(conf_int_simu$m == m)] <- c(tmb_gh$lambda + q95_norm * tmb_gh$lambda_std_error,
                                                        tmb_gh$gamma + q95_norm * tmb_gh$gamma_std_error,
                                                        tmb_gh$delta + q95_norm * tmb_gh$delta_std_error)
  ...
  for (idx_coverage in 1:COVERAGE_SAMPLES) {
    ...
    # Estimate a model
    coverage_model <- pois.HMM.generate_estimable_sample(ns = DATA_SIZE_SIMU,
                                                         mod = list(m = m,
                                                                    lambda = tmb_CI$lambda,
                                                                    gamma = tmb_CI$gamma,
                                                                    delta = tmb_CI$delta),
  ...
  conf_int_simu[conf_int_simu$m == m, ][1:(m ^ 2 + 2 * m), "Coverage.Profile"] <- coverage_count_profile$ratio
  conf_int_simu[conf_int_simu$m == m, ][1:(m ^ 2 + 2 * m), "Coverage.Bootstrap"] <- coverage_count_bootstrap$ratio
  conf_int_simu[conf_int_simu$m == m, ][1:(m ^ 2 + 2 * m), "Coverage.TMB"] <- coverage_count_tmb$ratio
}
...
for (i in 1:length(conf_int_simu[, 1])) {
  row <- conf_int_simu[i, c("Profile.L", "Profile.U")]
  conf_int_simu[i, c("Profile.L", "Profile.U")] <- cbind(min(row), max(row))
}
mllk_values_simu$m <- as.factor(as.numeric(mllk_values_simu$m))
mllk_values_simu$mllk <- as.numeric(mllk_values_simu$mllk)
mllk_values_simu$AIC <- as.numeric(mllk_values_simu$AIC)
mllk_values_simu$BIC <- as.numeric(mllk_values_simu$BIC)
conf_int_simu$m <- as.integer(conf_int_simu$m)

# Remove gamma and delta when m = 1 because they're pointless
if (1 %in% M_LIST_SIMU) {
  conf_int_simu <- conf_int_simu[- c(2,3), ]
}

estim_benchmarks_df_simu$m <- factor(estim_benchmarks_df_simu$m, levels = M_LIST_SIMU)