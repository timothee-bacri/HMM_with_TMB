...
if (!anyNA(working_conf_int$lower[tgamma_indices])) {
  ...
  conf_int_simu2$Profile.L[which(conf_int_simu2$m == m)][gamma_indices] <- natural_gamma
}
# Lambda (m values)
conf_int_simu2$Profile.U[which(conf_int_simu2$m == m)][lambda_indices] <- exp(working_conf_int$upper[lambda_indices])
# Gamma (m^2-m working parameters, m^2 natural ones)
if (!anyNA(working_conf_int$upper[tgamma_indices])) {
  natural_gamma <- as.numeric(gamma.w2n(m,
                                        working_conf_int$upper[tgamma_indices]))
  conf_int_simu2$Profile.U[which(conf_int_simu2$m == m)][gamma_indices] <- natural_gamma
}

# Bootstrap ---------------------------
set.seed(FIRST_SEED + 2)
if (BOOTSTRAP_SAMPLES != 0) {
  notif("Simu2, Bootstrap start")
  registerDoParallel(cores = CORES)
...
coverage_count_profile <- coverage_count_bootstrap <- coverage_count_tmb <- data.frame(parameter = parameter_names,
                                                                                       ...
                                                                                       ratio = 0)
idx_coverage <- 0
notif("Simu2, Coverage start")
begin_coverage <- Sys.time()
gamma_31_profile_ci <- data.frame(estimate = numeric(),
                                  lower = numeric(),
                                  upper = numeric())
while (idx_coverage < COVERAGE_SAMPLES) {
  ...
  for (reason in c("state_number", "TMB_null", "TMB_converge", "TMB_G_null",
    ...
    coverage_skips_simu2[coverage_skips_simu2$m == m, reason] <- coverage_skips_simu2[coverage_skips_simu2$m == m, reason] + coverage_model$failure[reason]
  }
  ...
  if (any(test_null == TRUE) | any(test_finite == FALSE)) {
    idx_coverage <- idx_coverage - 1
    coverage_skips_simu2[coverage_skips_simu2$m == m, "profile"] <- coverage_skips_simu2[coverage_skips_simu2$m == m, "profile"] + 1
    next
  }
  ...
  gamma_31_profile_ci[idx_coverage, 1] <- coverage_model$natural_parameters$gamma[3, 1]
  gamma_31_profile_ci[idx_coverage, 2] <- real_gamma_profile_lower[3]
  gamma_31_profile_ci[idx_coverage, 3] <- real_gamma_profile_upper[3]
  ...
  if (percent %% 10 == 0 && percent != 100) {
    ...
    notif(paste0("Simu2\n",
                 ...
                 "\nm: ", m,
                 "\nSize: ", DATA_SIZE_SIMU2,
                 "\nCoverage samples: ", COVERAGE_SAMPLES))
  }
}
notif("Simu2, Coverage end")
...
conf_int_simu2[conf_int_simu2$m == m, ][gamma_indices, ] <- conf_int_simu2[conf_int_simu2$m == m, ][gamma_indices, ][new_gamma_indices_truncated_table, ]
