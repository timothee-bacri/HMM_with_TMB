...
if (tmb_g$convergence != 0) {
  stop(paste("tmb_g didn't converge properly, simu2 dataset, m =",
             m))
}
if (tmb_h$convergence != 0) {
  stop(paste("tmb_h didn't converge properly, simu2 dataset, m =",
             m))
}
if (tmb_gh$convergence != 0) {
  stop(paste("tmb_gh didn't converge properly, simu2 dataset, m =",
             m))
}
...
conf_int_simu2[indices, "Estimate"] <- unlist(tmb_gh[PARAMS_NAMES])
conf_int_simu2[indices, "True.value"] <- as.numeric(c(true_lambda,
                                                      true_gamma,
                                                      true_delta))
...
if (BENCHMARK_SAMPLES != 0) {
  for (idx_counter in 1:BENCHMARK_SAMPLES) {
    ...
    benchmark_model <- pois.HMM.generate.estimable.sample(ns = DATA_SIZE_SIMU2,
                                                          mod = list(m = m,
                                                                     lambda = tmb_gh$lambda,
                                                                     gamma = tmb_gh$gamma,
                                                                     delta = tmb_gh$delta),
    ...
    estim_benchmarks_df_simu2 <- rbind(estim_benchmarks_df_simu2,
...
# Lambda (m values)
conf_int_simu2$Profile.L[lambda_indices] <- exp(working_conf_int$lower[lambda_indices])
...
if (!anyNA(working_conf_int$lower[tgamma_indices])) {
  ...
  conf_int_simu2$Profile.L[gamma_indices] <- natural_gamma
}
# Lambda (m values)
conf_int_simu2$Profile.U[lambda_indices] <- exp(working_conf_int$upper[lambda_indices])
# Gamma (m^2-m working parameters, m^2 natural ones)
if (!anyNA(working_conf_int$upper[tgamma_indices])) {
  natural_gamma <- as.numeric(gamma.w2n(m,
                                        working_conf_int$upper[tgamma_indices]))
  conf_int_simu2$Profile.U[gamma_indices] <- natural_gamma
}
...
if (BOOTSTRAP_SAMPLES != 0) {
  ...
  if (BOOTSTRAP_SAMPLES == 1) {
    names(bootstrap_simu2) <- params_names_latex
  } else {
    colnames(bootstrap_simu2) <- params_names_latex
  }
  q <- apply(bootstrap_simu2,
             2,
             quantile.colwise)
  conf_int_simu2$Bootstrap.L <- q[1, ]
  conf_int_simu2$Bootstrap.U <- q[2, ]
}
...
conf_int_simu2$TMB.L <- c(lambda_L,
                          gamma_L,
                          delta_L)
...
delta_U <- pmin(1,
                tmb_gh$delta + q95_norm * tmb_gh$delta_std_error)
conf_int_simu2$TMB.U <- c(tmb_gh$lambda + q95_norm * tmb_gh$lambda_std_error,
                          gamma_U,
                          delta_U)
...
while (idx_coverage < COVERAGE_SAMPLES) {
  ...
  coverage_model <- pois.HMM.generate.estimable.sample(ns = DATA_SIZE_SIMU2,
                                                       mod = list(m = m,
                                                                  lambda = true_lambda,
                                                                  gamma = true_gamma,
                                                                  delta = true_delta),
  ...
  for (reason in c("state_number", "TMB_null", "TMB_converge", "TMB_G_null",
    ...
    coverage_skips_simu2[coverage_skips_simu2$m == m, reason] <-
      coverage_skips_simu2[coverage_skips_simu2$m == m, reason] +
  ...
  if (any(test_null == TRUE) | any(test_finite == FALSE)) {
    idx_coverage <- idx_coverage - 1
    temp <- coverage_skips_simu2[coverage_skips_simu2$m == m, "profile"]
    coverage_skips_simu2[coverage_skips_simu2$m == m, "profile"] <- temp + 1
    next
  }
...
conf_int_simu2[1:(m ^ 2 + 2 * m), "Coverage.Profile"] <- coverage_count_profile$ratio * 100
conf_int_simu2[1:(m ^ 2 + 2 * m), "Coverage.Bootstrap"] <- coverage_count_bootstrap$ratio * 100
conf_int_simu2[1:(m ^ 2 + 2 * m), "Coverage.TMB"] <- coverage_count_tmb$ratio * 100
...
conf_int_simu2[lambda_indices, - 2] <- conf_int_simu2[lambda_indices, - 2][new_lambda_indices, ]
conf_int_simu2[gamma_indices, - 2] <- conf_int_simu2[gamma_indices, - 2][new_gamma_indices, ]
conf_int_simu2[delta_indices, - 2] <- conf_int_simu2[delta_indices, - 2][new_delta_indices, ]

# Reorder the TPM row-wise instead of column-wise
# Lexicographical parameter sort for gamma (sort on the parameter name)
new_gamma_indices_truncated_table <- order(conf_int_simu2[gamma_indices, "Parameter"])
# Replace rows by sorted rows
conf_int_simu2[gamma_indices, ] <- conf_int_simu2[gamma_indices, ][new_gamma_indices_truncated_table, ]
...
estim_benchmarks_df_simu2$m <- factor(estim_benchmarks_df_simu2$m,
                                      levels = M_LIST_SIMU2)
