...
parameter_names <- c(parameter_names,
                     paste0(rep("delta",
                                m),
                            1:m))
coverage_count_profile_simu3 <- coverage_count_bootstrap <- coverage_count_tmb <- data.frame(parameter = parameter_names,
                                                                                             count = 0,
                                                                                             ratio = 0,
                                                                                             missing = 0)
idx_coverage <- 0
notif("Simu3, Coverage start")
...
while (idx_coverage < COVERAGE_SAMPLES) {
  ...
  # If profiling doesn't yield results for all parameters, record it (because we don't guarantee profile CIs in this simulation)
  ...
  if (BOOTSTRAP_SAMPLES != 0) {
    ...
    bootstrap_simu3_df <- rbind(bootstrap_simu3_df, bootstrap_simu3)
  }
...
notif("Simu3, Coverage end")
coverage_count_profile_simu3[lambda_indices, "ratio"] <- coverage_count_profile_simu3[lambda_indices, "count"] / (COVERAGE_SAMPLES - coverage_count_profile_simu3[lambda_indices, "missing"])
coverage_count_profile_simu3[gamma_indices, "ratio"] <- coverage_count_profile_simu3[gamma_indices, "count"] / (COVERAGE_SAMPLES - coverage_count_profile_simu3[gamma_indices, "missing"])
coverage_count_profile_simu3[delta_indices, "ratio"] <- NA # delta is not a parameter for us, so it has no profile CI
...
conf_int_simu3[conf_int_simu3$m == m, ][1:(m ^ 2 + 2 * m), "Coverage.Profile"] <- coverage_count_profile_simu3$ratio * 100
