...
for (idx in 1:length(M_LIST_TINN)) {
  ...
  mllk_values_tinn[indexes, ] <- cbind(m = rep(m, 5),
  ...
  indexes <- (length(conf_int_tinn$m) + 1):(length(conf_int_tinn$m) + len_par)
  conf_int_tinn[indexes, "m"] <- m
  conf_int_tinn[indexes, "Parameter"] <- params_names_latex
  # Reminder, PARAMS_NAMES contains c("lambda", "gamma", "delta")
  conf_int_tinn[indexes, "Estimate"] <- unlist(tmb_CI[PARAMS_NAMES])
  ...
  temp <- microbenchmark("DM" = nlminb(parvect_init, pois.HMM.mllk, x_alias = tinn_data, m_alias = m)$convergence==0,
                         ...
                         control = list(order = "block", warmup = CONSISTENCY_BENCHMARK_TINN / 4),
                         setup = obj_init <<- MakeADFun(TMB_data, working_params_init, DLL = "poi_hmm", silent = TRUE),
                         check = "equal")
  
  # print(rbind(nlminb(parvect_init, pois.HMM.mllk, x_alias = tinn_data, m_alias = m)$par,
  #       nlminb(obj_init$par, obj_init$fn)$par,
  #       nlminb(obj_init$par, obj_init$fn, gradient = obj_init$gr)$par,
  #       nlminb(obj_init$par, obj_init$fn, hessian = obj_init$he)$par,
  #       nlminb(obj_init$par, obj_init$fn, gradient = obj_init$gr, hessian = obj_init$he)$par),
  #       digits = 10)
  rbind(nlminb(parvect_init, pois.HMM.mllk, x_alias = tinn_data, m_alias = m)$objective,
        nlminb(obj_init$par, obj_init$fn)$objective,
        nlminb(obj_init$par, obj_init$fn, gradient = obj_init$gr)$objective,
        nlminb(obj_init$par, obj_init$fn, hessian = obj_init$he)$objective,
        nlminb(obj_init$par, obj_init$fn, gradient = obj_init$gr, hessian = obj_init$he)$objective)
  ...
  # Lambda (m values)
  conf_int_tinn$Profile.L[which(conf_int_tinn$m == m)][1:m] <- exp(working_conf_int$lower[1:m])
  ...
  if (!anyNA(working_conf_int$lower[(m + 1):(m ^ 2)])) {
    ...
    conf_int_tinn$Profile.L[which(conf_int_tinn$m == m)][(m + 1):(m ^ 2 + m)] <- natural_gamma
  }
  # Lambda (m values)
  conf_int_tinn$Profile.U[which(conf_int_tinn$m == m)][1:m] <- exp(working_conf_int$upper[1:m])
  # Gamma (m^2-m working parameters, m^2 natural ones)
  if (!anyNA(working_conf_int$upper[(m + 1):(m ^ 2)])) {
    natural_gamma <- as.numeric(Gamma_w2n(m, working_conf_int$upper[(m + 1):(m ^ 2)]))
    conf_int_tinn$Profile.U[which(conf_int_tinn$m == m)][(m + 1):(m ^ 2 + m)] <- natural_gamma
  }
  ...
  conf_int_tinn$TMB.L[which(conf_int_tinn$m == m)] <- c(tmb_gh$lambda - q95_norm * tmb_gh$lambda_std_error,
  ...
  conf_int_tinn[conf_int_tinn$m == m, ][1:(m ^ 2 + 2 * m), "Coverage.Profile"] <- coverage_count_profile$ratio
  conf_int_tinn[conf_int_tinn$m == m, ][1:(m ^ 2 + 2 * m), "Coverage.Bootstrap"] <- coverage_count_bootstrap$ratio
  conf_int_tinn[conf_int_tinn$m == m, ][1:(m ^ 2 + 2 * m), "Coverage.TMB"] <- coverage_count_tmb$ratio
}
