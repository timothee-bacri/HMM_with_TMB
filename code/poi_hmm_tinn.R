...
for (idx in 1:length(M_LIST_TINN)) {
  ...
  if (CONSISTENCY_BENCHMARK_TINN != 0) {
    ...
    temp <- microbenchmark("DM" = nlminb(parvect_init, pois_HMM_mllk, x_alias = tinn_data, m_alias = m)$convergence==0,
    ...
    iterations <- c(nlminb(parvect_init, pois_HMM_mllk, x_alias = tinn_data, m_alias = m)$iterations,
  ...
  if (BENCHMARK_SAMPLES != 0) {
    for (idx_counter in 1:BENCHMARK_SAMPLES) {
      ...
      benchmark_model <- pois_HMM_generate_estimable_sample(ns = DATA_SIZE_TINN,
                                                            mod = list(m = m,
      ...
      temp <- microbenchmark("DM" = pois_HMM_mllk(parvect_mllk, tinn_data, m),
  ...
  if (BOOTSTRAP_SAMPLES != 0) {
    for (idx_sample in 1:BOOTSTRAP_SAMPLES) {
      temp <- pois_HMM_generate_estimable_sample(ns = DATA_SIZE_TINN,
                                                 mod = list(m = m,
                                                            lambda = tmb_CI$lambda,
                                                            gamma = tmb_CI$gamma,
                                                            delta = tmb_CI$delta),
    ...
    q <- apply(bootstrap_tinn, 2, quantile_colwise)
  ...
  while (idx_coverage < COVERAGE_SAMPLES) {
    ...
    # Estimate a model
    coverage_model <- pois_HMM_generate_estimable_sample(ns = DATA_SIZE_TINN,
                                                         mod = list(m = m,
                                                                    lambda = tmb_CI$lambda,
                                                                    gamma = tmb_CI$gamma,
                                                                    delta = tmb_CI$delta),
