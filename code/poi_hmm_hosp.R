...
for (idx in 1:length(M_LIST_HOSP)) {
  ...
  mllk_values_hosp[indices, ] <- cbind(m = rep(m, 5),
  ...
  indices <- (length(conf_int_hosp$m) + 1):(length(conf_int_hosp$m) + len_par)
  conf_int_hosp[indices, "m"] <- m
  conf_int_hosp[indices, "Parameter"] <- params_names_latex
  # Reminder, PARAMS_NAMES contains c("lambda", "gamma", "delta")
  conf_int_hosp[indices, "Estimate"] <- unlist(tmb_CI[PARAMS_NAMES])
  ...
  # Benchmarks ------------
  set.seed(19)
  if (BENCHMARK_SAMPLES_HOSP != 0) {
    for (idx_counter in 1:BENCHMARK_SAMPLES_HOSP) {
  ...
  working_conf_int <- foreach (idx_param = 1:len_w_par, .packages = all_packages, .inorder = TRUE, .combine = rbind) %dopar% {
  ...
  # Lambda (m values)
  conf_int_hosp$Profile.L[which(conf_int_hosp$m == m)][lambda_indices] <- exp(working_conf_int$lower[lambda_indices])
  ...
  if (!anyNA(working_conf_int$lower[tgamma_indices])) {
    ...
    conf_int_hosp$Profile.L[which(conf_int_hosp$m == m)][gamma_indices] <- natural_gamma
  }
  # Lambda (m values)
  conf_int_hosp$Profile.U[which(conf_int_hosp$m == m)][lambda_indices] <- exp(working_conf_int$upper[lambda_indices])
  # Gamma (m^2-m working parameters, m^2 natural ones)
  if (!anyNA(working_conf_int$upper[tgamma_indices])) {
    natural_gamma <- as.numeric(Gamma_w2n(m, working_conf_int$upper[tgamma_indices]))
    conf_int_hosp$Profile.U[which(conf_int_hosp$m == m)][gamma_indices] <- natural_gamma
  }
  
  # Bootstrap ---------------------------
  set.seed(20)
  if (BOOTSTRAP_SAMPLES_HOSP != 0) {
    registerDoParallel(cores = detectCores() - 1)
    bootstrap_hosp <- foreach (idx_sample = 1:BOOTSTRAP_SAMPLES_HOSP, .packages = packages, .combine = rbind) %dopar% {
      TMB::compile("code/poi_hmm.cpp")
      dyn.load(dynlib("code/poi_hmm"))
  ...
  conf_int_hosp$TMB.L[which(conf_int_hosp$m == m)] <- c(lambda_L,
                                                        gamma_L,
                                                        delta_L)
  ...
  # Fixes -------------------------
  # Fix label switching in conf_int_hosp
  ...
  conf_int_hosp[conf_int_hosp$m == m, - 2][lambda_indices, ] <- conf_int_hosp[conf_int_hosp$m == m, - 2][lambda_indices, ][new_lambda_indices, ]
  conf_int_hosp[conf_int_hosp$m == m, - 2][gamma_indices, ] <- conf_int_hosp[conf_int_hosp$m == m, - 2][gamma_indices, ][new_gamma_indices, ]
  conf_int_hosp[conf_int_hosp$m == m, - 2][delta_indices, ] <- conf_int_hosp[conf_int_hosp$m == m, - 2][delta_indices, ][new_delta_indices, ]
  
  # Reorder the TPM row-wise instead of column-wise
  # Lexicographical parameter sort for gamma (sort on the parameter name)
  new_gamma_indices_truncated_table <- order(conf_int_hosp[conf_int_hosp$m == m, ][gamma_indices, "Parameter"])
  # Replace rows by sorted rows
  conf_int_hosp[conf_int_hosp$m == m, ][gamma_indices, ] <- conf_int_hosp[conf_int_hosp$m == m, ][gamma_indices, ][new_gamma_indices_truncated_table, ]
}
