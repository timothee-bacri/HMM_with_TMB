...
for (idx in 1:length(M_LIST_LAMB)) {
  ...
  if (BENCHMARK_SAMPLES != 0) {
    for (idx_counter in 1:BENCHMARK_SAMPLES) {
      ...
      benchmark_model <- pois_HMM_generate_estimable_sample(ns = DATA_SIZE_LAMB,
                                                            mod = list(m = m,
      ...
      temp <- microbenchmark("DM" = pois_HMM_mllk(parvect_mllk, lamb_data, m),
  ...
  if (BOOTSTRAP_SAMPLES != 0) {
    for (idx_sample in 1:BOOTSTRAP_SAMPLES) {
      temp <- pois_HMM_generate_estimable_sample(ns = DATA_SIZE_LAMB,
                                                 mod = list(m = m,
                                                            lambda = tmb_CI$lambda,
                                                            gamma = tmb_CI$gamma,
                                                            delta = tmb_CI$delta),
    ...
    q <- apply(bootstrap_lamb, 2, quantile_colwise)
  ...
  while (idx_coverage < COVERAGE_SAMPLES) {
    ...
    # Estimate a model
    coverage_model <- pois_HMM_generate_estimable_sample(ns = DATA_SIZE_LAMB,
                                                         mod = list(m = m,
                                                                    lambda = tmb_CI$lambda,
                                                                    gamma = tmb_CI$gamma,
                                                                    delta = tmb_CI$delta),
    ...
    for (reason in c("state_number", "TMB_null", "TMB_converge", "TMB_G_null",
      ...
      coverage_skips_lamb[coverage_skips_lamb$m == m, reason] <- coverage_skips_lamb[coverage_skips_lamb$m == m, reason] + coverage_model$failure[reason]
    }
