...
for (idx in 1:length(M_LIST_LAMB)) {
  ...
  while (idx_coverage < COVERAGE_SAMPLES) {
    ...
    # # In these cases, we reduce the total coverage_adjusted_sample_size_gamma_lamb[idx, "size"]
    ...
    #   coverage_adjusted_sample_size_gamma_lamb[idx, "size"] <- coverage_adjusted_sample_size_gamma_lamb[idx, "size"] - 1
    # } else {
  ...
  # coverage_count_profile[gamma_indices, "ratio"] <- coverage_count_profile[gamma_indices, "count"] / coverage_adjusted_sample_size_gamma_lamb[idx, "size"]
