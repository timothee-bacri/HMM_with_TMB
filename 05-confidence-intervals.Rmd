...
parameters <- pois_HMM_pn2pw(m, lambda_init, gamma_init)
...
This function relies on the custom function `TMB_estimate` which is a wrapper of `nlminb` made for Poisson HMM estimation via TMB.
```{r pois_HMM_generate_estimable_sample}
```
...
for (idx_sample in 1:BOOTSTRAP_SAMPLES) {
  ...
  temp <- pois_HMM_generate_estimable_sample(ns = DATA_SIZE,
                                             mod = list(m = m,
                                                        lambda = lambda,
                                                        gamma = gamma),
...
However, a large number of bootstrap samples lowers that risk since we automatically discard 5\% of the most extreme values during the data generating process.
